...
export default function StateDeputies() {
  ...
  const {
    data: stateDeputiesData,
    refetch: stateDeputiesRefetch,
    isLoading: isStateDeputiesLoading,
    isRefetching: isStateDeputiesRefetching,
    error: stateDeputiesError,
  } = useQuery(
    `stateDeputies-${currentStateAbbreviation}`,
    async () =>
      (await api.get(getUrl('stateDeputy', currentStateAbbreviation ?? '')))
        .data,
    {
  ...
  const currentCandidates: ICandidate[] = useMemo(() => {
    if (!stateDeputiesData?.cand) return [];

    const candidates = stateDeputiesData?.cand.filter((candidate: ICandidate) =>
  ...
  const validVotes = Number(stateDeputiesData?.vv ?? 0);
  const nullVotes = Number(stateDeputiesData?.vn ?? 0);
  const whiteVotes = Number(stateDeputiesData?.vb ?? 0);

  function handleRefetch() {
    if (selectedState) {
      stateDeputiesRefetch();
    }
  }

  useEffect(() => {
    if (stateDeputiesError)
  ...
  return (
    <Flex flexDir='column' flex={1} mb='1rem'>
      ...
      <Progress
        data={stateDeputiesData}
        isLoading={isStateDeputiesLoading || isStateDeputiesRefetching}
            ...
            >
              ...
              <Flex p='1rem' flexDir='column' align='center'>
                <Box h='150px' w='150px' my='1.5rem'>
                  ...
                  >
                    {isStateDeputiesLoading || isStateDeputiesRefetching ? (
                      <>
