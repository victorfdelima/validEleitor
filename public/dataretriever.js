...
async function getVariableFile(city, uf, role) {
  ...
  const filepath =
    ...
    `${host}/${ciclo}/divulgacao/${ambiente}/${codigoEleicao}/dados/${uf}/${uf}${String(
      cityCode
    ).padStart(5, "0")}-c${roleCode}-e${codigoEleicao.padStart(6, "0")}-v.json`;
...
async function getFixedFile(filename) {
  const filepath =
    // eslint-disable-next-line max-len
    `${host}/${ciclo}/divulgacao/${ambiente}/${codigoEleicao}/dados/${filename.slice(
      0,
      2
    )}/${filename}.json`;
...
async function getFiles() {
  ...
  const matematicamenteCache = await JSON.parse(
    localStorage.getItem("matematicamente")
  );
  ...
  for (const city of cities) {
    ...
    if (
      ...
      notifyElection("ELEIÇÃO!", `Prefeito(a) eleito(a) em ${city.name}`);
    } else if (
    ...
    if (
      ...
      notifyElection("ELEIÇÃO!", `vereador(a) eleito(a) em ${city.name}`);
    } else if (
  ...
  console.log(
    `MATEMATICAMENTE: ${JSON.stringify(localStorage.matematicamente)}`
  );
  changedForm();
}
...
function getCandidateByNumber(number, fixedFile) {
  // NO AGUARDO DO TSE SYNVAL FALAR COM ELES
